...
This project demonstrates how to deploy the classic 2048 game on an AWS EKS cluster using kubectl, eksctl, and the AWS CLI.

## Step 1: Create an EKS Cluster
### Run the following command to create a fully-managed EKS cluster with Fargate profiles:
```bash
eksctl create cluster --name demo-cluster --region us-east-1 --fargate
```
#### This command sets up a serverless EKS cluster, so your pods will run on Fargate rather than EC2 instances.
...
* Managed by AWS, includes **API server**, **etcd**, **scheduler**, and **controller-manager**.
...
* Creates a **VPC** (if not provided) with **subnets**(public or private, usually private for Fargate), **Route tables**, and **Internet/NAT gateways**.
...
* `eksctl` eksctl uses **CFN** to manage cluster resources and Fargate profiles.

#### 7. **aws-auth ConfigMap**
